...
export const AdminSidebar: React.FC<AdminSidebarProps> = ({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 max-w-[85vw] bg-[#0F172A] border-r border-slate-800 flex flex-col justify-between h-full select-none text-slate-300 transform transition-transform duration-200 ease-in-out lg:static lg:translate-x-0 lg:w-64 lg:h-screen lg:shrink-0 lg:z-auto ${
        ...
        <div>
          <div className="p-4 sm:p-5 border-b border-slate-800/80 flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={onCloseMobile}
              className="lg:hidden p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 cursor-pointer touch-target flex items-center justify-center"
            ...
            {visibleNavItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-3 rounded-xl text-[13.5px] font-medium transition-all cursor-pointer min-h-[44px] ${
                    isActive
