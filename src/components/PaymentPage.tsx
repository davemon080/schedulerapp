...
export const PaymentPage: React.FC<PaymentPageProps> = ({
  ...
  // 1. Student Session Discovery
  const [student, setStudent] = useState<any>(() => {
    if (initialUserSession) return initialUserSession;
    try {
  ...
  const [activeSemesterCode, setActiveSemesterCode] = useState<string>('1st Semester 2025/2026');
  const [semesterFee, setSemesterFee] = useState<number>(2000);

  // Billing Contact inputs (editable to ensure Paystack sends receipt to student's preferred email)
  const [billingEmail, setBillingEmail] = useState<string>('');
  const [billingPhone, setBillingPhone] = useState<string>('');

  // Student manual lookup fallback (if arriving directly without session)
  ...
  // Manual student enrollment form (if new student not found in DB)
  ...
  // Payment State Machine: 'idle' | 'initiating' | 'redirecting' | 'verifying' | 'success' | 'cancelled' | 'error'
  ...
  // Determine whether current student is already paid
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (semConfig?.semester_code && !isCancelled) {
          setActiveSemesterCode(semConfig.semester_code);
        }

        // Identify student from URL param, initial session, or localStorage
        const params = new URLSearchParams(window.location.search);
        const urlStudent = params.get('student') || params.get('matric') || params.get('email');
        ...
        if (sessionIdentifier) {
          const freshStudent = await fetchStudentByEmailOrMatric(sessionIdentifier);
          if (freshStudent && !isCancelled) {
            setStudent((prev: any) => ({
              ...
              matricNumber:
                ...
                prev?.matricNumber ||
                sessionIdentifier,
              department: freshStudent.department || prev?.department || 'Industrial Chemistry',
              level: freshStudent.level || prev?.level || 100,
              email: freshStudent.email || prev?.email,
            }));
          } else if (urlStudent && !isCancelled) {
            // URL provided a matric or email, create temporary student object if not found in db yet
            setStudent({
              fullName: 'Student',
              matricNumber: urlStudent,
              department: 'Industrial Chemistry',
              level: 100,
              email: urlStudent.includes('@') ? urlStudent : `${urlStudent.replace(/[^a-zA-Z0-9]/g, '').toLowerCase()}@university.edu`,
            });
          }
        }
      } catch (e) {
        console.warn('Payment page initial load notice:', e);
      } finally {
  ...
  // Manual student matric search
  const handleManualMatricLookup = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      if (studentDoc) {
        ...
        setManualLookupError('No existing account found with this Matriculation Number. You can enter your details below to register.');
  ...
  // Manual student enrollment submit
  ...
  // Verify transaction with backend & activate semester in Firestore
  const handleVerifyPayment = async (reference: string) => {
    ...
    try {
      ...
      if (!verifyData.status || !verifyData.success) {
        setPaymentStatus('error');
        setErrorMessage(verifyData.message || 'Payment verification failed or transaction was not successful.');
        return;
      }

      // Step 2: Record in Firestore
      ...
      if (result.success) {
        ...
        setStatusMessage('Payment verified successfully! Full semester access is now active.');
        ...
        setPaymentStatus('error');
        setErrorMessage(result.error || 'Payment was received, but activating access encountered an error. Please retry.');
      }
    } catch (err: any) {
      console.error('Payment verification error:', err);
      setPaymentStatus('error');
      setErrorMessage(err?.message || 'A network error occurred while verifying payment. Please click retry below.');
    }
  };

  // REDIRECT TO PAYSTACK CHECKOUT (Primary Official Flow)
  const handleRedirectToPaystack = async () => {
    ...
    setStatusMessage('Initializing secure transaction with Paystack...');
    ...
    const studentDept = student.department || 'Industrial Chemistry';
    ...
    try {
      ...
      if (data.status && data.data?.authorization_url) {
        ...
        // Immediate redirection to Paystack hosted payment page
        window.location.href = data.data.authorization_url;
      } else {
        setPaymentStatus('error');
        setErrorMessage(data.message || 'Could not initialize Paystack transaction. Please check your network and retry.');
      }
    } catch (err: any) {
      console.error('Paystack initialization error:', err);
      setPaymentStatus('error');
      setErrorMessage(err?.message || 'Error connecting to Paystack gateway.');
    }
  };

  // INLINE POPUP (Alternative Flow for Desktop users)
  const handleLaunchInlinePaystack = () => {
    ...
    setErrorMessage(null);
    setPaymentStatus('initiating');
    setStatusMessage('Launching Paystack payment window...');
    ...
    // Fallback if inline script failed
    handleRedirectToPaystack();
  };
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] text-[#1C1C1E] flex flex-col items-center justify-start px-3.5 sm:px-4 py-4 sm:py-7 relative overflow-x-hidden select-none safe-area-top safe-area-bottom">
      {/* Ambient background soft blur orbs */}
      ...
      {/* Main Container - optimized for mobile WebView and desktop browser */}
      <div className="w-full max-w-lg mx-auto flex flex-col space-y-4">
        {/* Navigation Bar / Return to Scheduler */}
        <div className="flex items-center justify-between">
          <button
            onClick={handleReturnToScheduler}
            ...
            title="Return to Scheduler application"
          >
        ...
        <AnimatePresence>
          {paymentStatus === 'redirecting' && (
            <motion.div
              ...
              className="glass-container rounded-[26px] p-6 bg-white/95 border border-blue-200 shadow-xl text-center space-y-4"
            >
        ...
        {/* =========================================================================
            STATE: SUCCESS RECEIPT & ACTIVATION CELEBRATION
            ========================================================================= */}
        <AnimatePresence>
          {paymentStatus === 'success' && (
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              className="glass-container rounded-[28px] p-5 sm:p-6 bg-white/95 border border-emerald-300 shadow-xl text-center space-y-4"
            >
              ...
              {/* Primary Action: Return to Scheduler App */}
              <div className="space-y-2.5 pt-1">
                <button
                  id="btn-return-to-scheduler-success"
                  onClick={handleReturnToScheduler}
                ...
                >
                  <span>Open Unlocked Scheduler</span>
        ...
        {paymentStatus === 'idle' && student && isAlreadyPaid && (
          <motion.div
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            className="glass-container rounded-[28px] p-5 sm:p-6 border border-emerald-200 bg-white/95 text-center space-y-4 shadow-sm"
          >
            ...
            <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-left text-[12.5px] space-y-1.5">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-slate-800">{student.department || 'Industrial Chemistry'}</span>
              </div>
              <div className="flex justify-between">
        ...
        {!student && !isLoadingStudent && paymentStatus === 'idle' && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="glass-container rounded-[26px] p-5 border border-white shadow-sm space-y-4 bg-white/95"
          >
            ...
            {!showManualCreate ? (
              <form onSubmit={handleManualMatricLookup} className="space-y-3">
                <div>
        ...
        {paymentStatus === 'idle' && student && !isAlreadyPaid && (
          ...
          >
            ...
            <div className="glass-container rounded-[24px] p-4 bg-white/95 border border-white shadow-xs">
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-slate-500 truncate">
                    {student.department || 'Industrial Chemistry'} • {student.level || 100} Level
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="glass-container rounded-[26px] p-4.5 bg-white/95 border border-white shadow-xs space-y-3.5">
              ...
              {/* Billing Contact (Editable so student receives Paystack receipt) */}
              ...
              <div className="pt-2 border-t border-slate-100">
                <span className="block text-[10.5px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">
                  Supported Payment Channels:
                </span>
                <div className="flex flex-wrap items-center gap-1.5 text-[10.5px] text-slate-600 font-semibold">
                  <span className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200/80">Mastercard</span>
                  <span className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200/80">Visa</span>
                  <span className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200/80">Verve</span>
                  <span className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200/80">Bank Transfer</span>
                  <span className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200/80">USSD (*737#)</span>
                  <span className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200/80">OPay</span>
                </div>
              </div>
            </div>

            {/* PRIMARY REDIRECT TO PAYSTACK BUTTON */}
            <div className="space-y-2 pt-1">
              ...
              <div className="flex items-center justify-center gap-1.5 text-[11.5px] text-slate-500 pt-0.5">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
                <span>You will be securely redirected to Paystack's official page to pay.</span>
              </div>

              {/* Secondary Option: In-page Modal */}
              <div className="text-center pt-2">
                <button
                  type="button"
                  onClick={handleLaunchInlinePaystack}
                  className="text-[12px] font-bold text-slate-600 hover:text-[#007AFF] transition-colors cursor-pointer"
                >
                  Prefer in-page pop-up? Click to pay without redirecting
                </button>
              </div>
            </div>
          </motion.div>
        ...
        {/* Footer Guarantee */}
        <div className="pt-3 text-center text-[11px] text-slate-400 space-y-1">
          ...
          <p>© 2025/2026 Academic Session • University Scheduler System</p>
        </div>
      </div>
    </div>
  );
